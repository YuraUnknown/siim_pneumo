...
from encoding.nn import GlobalAvgPool2d
...
class EncNet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=True, lateral=False,
        ...
        if backbone == 'resnet18':
            self.head = EncHead(512, self.nclass, se_loss=se_loss,
            ...
            self.head = EncHead(448, self.nclass, se_loss=se_loss,
                            lateral=lateral, norm_layer=norm_layer,
                            up_kwargs=self._up_kwargs)
        else:    
        ...
        if aux:
            if backbone == 'resnet18':
                self.auxlayer = FCNHead(256, nclass, norm_layer=norm_layer)
            elif backbone == 'resnet34':
                self.auxlayer = FCNHead(256, nclass, norm_layer=norm_layer)
            elif backbone == 'efficient':
                self.auxlayer = FCNHead(160, nclass, norm_layer=norm_layer)
            else:
